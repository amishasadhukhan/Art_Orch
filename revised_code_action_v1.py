...
Each shape in the step already has a "type" field — map it directly to the Krita API call:
  ...
  stroke         → for x1,y1,x2,y2 in stroke_list:
                       node.paintLine(QPointF(x1,y1), QPointF(x2,y2), 0.9, 0.3, "ForegroundColor")
...
2. Always start with EXACTLY these imports (correct modules — do not guess):
   ...
   NOTE: QPainterPath is in QtGui, NOT QtCore — QRectF/QPointF are QtCore,
   QColor/QPainterPath are QtGui. Getting this wrong crashes the script with
   an ImportError before anything paints.
...
6. COORDINATES: exact coordinates per shape are pre-computed and given to you.
   ...
   iterate over it and call paintLine for each tuple.
...
8. Choose the brush preset whose description best matches the element's look
   ...
   Use the EXACT preset name as it appears in that list, including any
   category prefix like "b) " or "c) " — do not shorten, paraphrase, or
   invent a name. A wrong/nonexistent name makes setCurrentBrushPreset()
   silently do nothing, leaving whatever brush was active from a PREVIOUS
   script still active (Krita Scripter scripts run in the same live session,
   so brush state persists across separately-run scripts).
